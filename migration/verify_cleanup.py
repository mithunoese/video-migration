...
Source content in Kaltura is never deleted — the client closes their own account.

Usage (via run.py):
    python run.py verify              # verify all completed entries
    python run.py verify --id 1_abc   # single entry
"""
...
def run_verify_cleanup(
    ...
    Source content in Kaltura is never touched — client closes their own account.
